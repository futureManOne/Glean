...
const svgPath = path.resolve('store-assets/brand-icon.svg');
const publicIconDir = path.resolve('public/icon');

if (!fs.existsSync(publicIconDir)) {
  fs.mkdirSync(publicIconDir, { recursive: true });
}

// Master Optimized SVG for Glean (拾句)
...
// Save master SVG to store-assets/brand-icon.svg
fs.writeFileSync(svgPath, masterSvg, 'utf8');
console.log('Saved master SVG to', svgPath);

// Render production PNGs
...
for (const size of sizes) {
  ...
  await page.setContent(`<!DOCTYPE html><html><head><style>*{margin:0;padding:0;overflow:hidden;background:transparent;}</style></head><body><div style="width:${size}px;height:${size}px;display:flex;">${masterSvg}</div></body></html>`);
  
  const destPng = path.join(publicIconDir, `${size}.png`);
  await page.screenshot({ path: destPng, omitBackground: true });
  console.log(`Generated ${destPng} (${size}x${size})`);
  await page.close();
}
